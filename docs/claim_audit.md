...
- The planner suite includes random open-loop, best-of-N, UCT MCTS, value-guided MCTS, uncertainty-penalized MCTS, and conservative-backup MCTS.
- The diagnostics measure selected-return optimism, model reward bias, uncertainty exposure, transition error, depth-wise bias, and search concentration.
- In `results/full/manifest.json`, UCT MCTS has a larger mean selected-return optimism gap than best-of-N at budget 1024 (`0.686` versus `0.301`).
- In the same full run, the best calibrated repair (`uncertainty_mcts`) reduces the gap to `0.236`.
